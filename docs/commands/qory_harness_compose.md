...
```
      --check            compare the home with what the stack and modules define now and write nothing; exit 6 when a file or link differs. The links from the checkout into the home and the report are not compared
