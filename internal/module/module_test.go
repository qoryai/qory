...
// tree writes a module under a fresh temporary directory and returns it. A key ending in "/"
...
// entryLines renders the entries as "kind/name path" lines, with the module directory cut off
// the path, so a test can compare them as one string.
func entryLines(l *Module) string {
...
// TestReadListsEveryKindWithItsPaths reads a module holding all six kinds, the settings
...
func TestReadRefusesADirectoryUnderHooks(t *testing.T) {
	...
	if err == nil {
		t.Fatal("read a module with a directory under hooks")
	}
	want := "module core: hooks/scripts is a directory; a hook is one file. Keep its helpers elsewhere in the module, reached as $QORY_HARNESS_HOME/modules/core/<path>"
	if err.Error() != want {
...
// elsewhere in the module, and the message under a variant names the directory the variant
// reads.
func TestReadRefusesALinkToADirectoryUnderHooks(t *testing.T) {
	...
	want := "module core: claude/hooks/scripts is a directory; a hook is one file. Keep its helpers elsewhere in the module, reached as $QORY_HARNESS_HOME/modules/core/<path>"
...
func TestReadRefusesASkillWithoutItsSkillFile(t *testing.T) {
	...
	if want := "module core: skills/review has no SKILL.md"; err.Error() != want {
		t.Fatalf("error %q, want %q", err, want)
	}
}

// TestReadEmptyModule reads a directory holding no kind directory at all.
...
// TestReadRefusesAVariantThatLeavesTheModule refuses a variant naming an unknown kind or a
// directory outside the module.
func TestReadRefusesAVariantThatLeavesTheModule(t *testing.T) {
	cases := []struct {
		name    string
		variant Variant
		want    string
		...
		{"an absolute path", Variant{"agents": "/etc"}, `module multi: variant codex reads agents from "/etc", which is not a directory inside the module`},
	}
	for _, c := range cases {
...
// since every module names itself in one.
...
func TestReadManifestRefusesEnv(t *testing.T) {
	...
	cases := []struct {
		...
		{"qory's own variable", "  QORY_HARNESS_HOME: .\n", "env.QORY_HARNESS_HOME is qory's own; a module exports another name"},
		{"an empty value", `  CORE_HOME: ""` + "\n", "env.CORE_HOME:  is not a path inside the module"},
		{"an absolute value", "  CORE_HOME: /etc\n", "env.CORE_HOME: /etc is not a path inside the module"},
		{"the parent", "  CORE_HOME: ..\n", "env.CORE_HOME: .. is not a path inside the module"},
		{"a path leaving the module", "  CORE_HOME: scripts/../../other\n", "env.CORE_HOME: scripts/../../other is not a path inside the module"},
...
func TestSelectVariantPicksTheForcedRuntimeOrDefaultOne(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "the module root without variants", manifest: &Manifest{Name: "core"}, runtime: "claude", want: ""},
		{name: "the module root without a manifest", runtime: "claude", want: ""},
		{
			name: "a default of fail", manifest: failing, runtime: "gemini",
			wantErr: "the module has no variant for runtime gemini and its default is fail; variants: claude, codex",
		},
		{
			name: "no default at all", manifest: two, runtime: "gemini",
			wantErr: "the module has no variant for runtime gemini and declares no default; variants: claude, codex",
		},
		{
			name: "a forced variant the module has not", manifest: two, forced: "amp", runtime: "claude",
			wantErr: `variant "amp" is forced, and the module has no such variant; variants: claude, codex`,
		},
		{
			name: "a forced variant on a module without variants", manifest: &Manifest{Name: "core"}, forced: "codex", runtime: "claude",
			wantErr: `variant "codex" is forced, and the module declares no variants`,
		},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
...
// directory the process cannot list, with the module named.
func TestReadPropagatesADirectoryError(t *testing.T) {
	...
	for _, kind := range []string{"skills", "agents", "commands", "output-styles", "hooks", "mcp", "settings"} {
		t.Run(kind, func(t *testing.T) {
			...
			if !strings.HasPrefix(err.Error(), "module core: ") {
