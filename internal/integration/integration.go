...
// environment and no standard input. program is a path. It refuses a program that
// does not exit 0 within [DescribeWait], giving the one line it wrote on standard
...
func Describe(ctx context.Context, program string) (*Description, error) {
	...
	// A child left holding the output is not waited for past the limit.
...
func read(doc map[string]any) (*Description, error) {
	...
	// settings' properties are all there is to look through.
...
// the settings written $, JSON's escape for it, since the runner replaces
// ${argument} wherever it stands in an argument and a setting reaches the program as
// it was written.
func CredentialAdapter(program string, settings []byte) []string {
	return []string{program, RoleCredential, "--settings", strings.ReplaceAll(string(settings), "$", `$`), "--", "${argument}"}
}
...
func broken(e *jsonschema.ValidationError) []string {
	...
	case *kind.PropertyNames:
		return []string{at + "." + k.Property + " is not a setting it takes"}
...
// that it did, so a program that prints without end holds no more than that.
type capped struct {
	bytes.Buffer
	over bool
}
