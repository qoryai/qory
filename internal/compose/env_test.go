...
func TestTwoModulesExportingOneNameIsRefused(t *testing.T) {
	...
	res, err := compose.ComposeWith(p, compose.Options{Env: map[string]string{"HARNESS_HOME": "$QORY_HARNESS_HOME/modules/b", "PROFILE": "nextjs"}})
	if err != nil {
		t.Fatal(err)
	}
	if res.Env["HARNESS_HOME"] != "$QORY_HARNESS_HOME/modules/b" || res.Env["PROFILE"] != "nextjs" {
		t.Errorf("env: %v", res.Env)
	}
}
