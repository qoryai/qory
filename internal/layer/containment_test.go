...
func TestReadRefusesMCPValuesOfTheWrongType(t *testing.T) {
	for _, c := range []struct{ body, want string }{
		{`{"command": "x", "args": [30]}`, "layer core: mcp/db.json: args holds 30, which is not a string"},
		{`{"command": "x", "env": {"TIMEOUT": 30}}`, "layer core: mcp/db.json: env.TIMEOUT is 30, which is not a string"},
		{`{"command": "x", "type": "grpc"}`, "layer core: mcp/db.json: type grpc is not stdio, http or sse"},
		{`{"command": ""}`, "layer core: mcp/db.json: command is not a non-empty string"},
		{`{"command": "x", "description": 3}`, "layer core: mcp/db.json: description is 3, which is not a string"},
		{`{"command": "x"} trailing`, "layer core: mcp/db.json does not hold a JSON object: invalid character 't' after top-level value"},
	} {
...
// TestReadRefusesAPathThatLeavesTheLayer is an entry, a fragment, AGENTS.md or a file
// inside a skill that is a symlink to a file or directory outside the layer: the harness
// would read what the report does not show, so the compose refuses and names the target.
func TestReadRefusesAPathThatLeavesTheLayer(t *testing.T) {
	outside := t.TempDir()
	...
	for _, c := range []struct{ link, target, want string }{
		{"AGENTS.md", filepath.Join(outside, "secret"), "layer core: AGENTS.md links to "},
		{"agents/reviewer.md", filepath.Join(outside, "secret"), "layer core: agents/reviewer links to "},
		{"hooks/guard.sh", filepath.Join(outside, "secret"), "layer core: hooks/guard.sh links to "},
		{"skills/s", filepath.Join(outside, "skill"), "layer core: skills/s links to "},
		{"skills/ok/notes.md", filepath.Join(outside, "secret"), "layer core: skills/ok/notes.md links to "},
		{"skills/ok/lib/helper.py", filepath.Join(outside, "secret"), "layer core: skills/ok/lib/helper.py links to "},
		{"skills/ok/vendor", outside, "layer core: skills/ok/vendor links to "},
		{"settings/claude/settings.json", filepath.Join(outside, "secret"), "layer core: settings/claude/settings.json links to "},
	} {
		...
		_, err := Read("core", dir, nil, "")
		if err == nil || !strings.HasPrefix(err.Error(), c.want) || !strings.HasSuffix(err.Error(), ", outside the layer") {
			t.Errorf("%s: error %v, want it to start with %q", c.link, err, c.want)
		}
	}
	// A link inside the layer is fine, at the root and inside a skill.
	...
	if _, err := Read("core", dir, nil, ""); err != nil {
		t.Errorf("a link inside the layer was refused: %v", err)
	}
}
...
func TestReadRefusesADanglingLinkInsideASkill(t *testing.T) {
	...
	_, err := Read("core", dir, nil, "")
	if err == nil || !strings.HasPrefix(err.Error(), "layer core: skills/ok/notes.md ") {
...
// in the layer: it reads like the directory.
...
// is inside the layer however it reads.
