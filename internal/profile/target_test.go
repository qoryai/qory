...
// writeProfile writes a profile whose target block is the given YAML and loads it.
func writeProfile(t *testing.T, target string) (*Profile, error) {
	t.Helper()
	dir := t.TempDir()
	if err := os.MkdirAll(filepath.Join(dir, "layers", "core"), 0o755); err != nil {
		t.Fatal(err)
	}
...
// TestTargetRefuses covers every target a profile may not carry.
...
// TestRuntimesValidateIsCalledOnTheFlagToo pins that the command layer can validate what
// --runtime put in place of the profile's target.
