...
// TestNewCarriesLinksEnvAndExtensions is a result with a linked layer, an exported
...
func TestNewCarriesLinksEnvAndExtensions(t *testing.T) {
	...
	res := result()
	res.Layers[0].Link = "harness"
	res.Env = map[string]string{"HARNESS_HOME": "$QORY_HARNESS_HOME/layers/core"}
	res.Profile.Extensions = map[string]map[string]any{"acme": {"sweep_floor": 160, "roots": []any{"scripts"}}}
	r := report.New(res, "app", "/work/app", "/work/app/.qory/harness")
	if r.Layers[0].Link != "harness" || r.Env["HARNESS_HOME"] != "$QORY_HARNESS_HOME/layers/core" || r.Extensions["acme"]["sweep_floor"] != 160 {
		t.Errorf("report: %+v", r)
	}
	...
	for _, want := range []string{"linked as harness", "Env", "HARNESS_HOME  $QORY_HARNESS_HOME/layers/core", "Extensions", "acme.roots        [\"scripts\"]", "acme.sweep_floor  160"} {
