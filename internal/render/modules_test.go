...
// linkModule sets the checkout-root name the stack links the named module as, the way
// the stack loader does, and returns the module.
func linkModule(t *testing.T, res *compose.Result, module, link string) compose.Module {
	t.Helper()
	for i := range res.Modules {
		if res.Modules[i].Name == module {
			res.Modules[i].Link = link
			return res.Modules[i]
		}
	}
	t.Fatalf("no module %s in the fixture", module)
...
// TestModuleLinkReachesTheModuleThroughTheHome is a stack that links the core module as
// harness: the checkout gets a relative link resolving to the module's directory through
// the home's modules/core, listed in the exclude file, and a later compose that drops the
// link from the stack takes it and its line back.
func TestModuleLinkReachesTheModuleThroughTheHome(t *testing.T) {
	res, root, home := composeFixture(t, "claude")
	claude := lookup(t, "claude")
	core := linkModule(t, res, "core", "harness")
	...
	linked, err := render.LinkModules(res, root, home, nil, false)
	...
	if err != nil || target != filepath.Join(".qory", "harness", "modules", "core") {
	...
	if _, err := os.Stat(filepath.Join(path, "scripts", "db.py")); err != nil {
		t.Errorf("the module's own file is not reachable through the link: %v", err)
	}
	...
	linked, err = render.LinkModules(res, root, home, []string{"harness"}, false)
	if err != nil || len(linked.Skipped) != 0 || len(linked.Replaced) != 0 {
	...
	// The stack drops the link: the earlier compose's name is pruned.
	linkModule(t, res, "core", "")
	if _, err := render.LinkModules(res, root, home, []string{"harness"}, false); err != nil {
		t.Fatal(err)
	}
	if _, err := os.Lstat(path); err == nil {
		t.Error("harness is still there after the stack dropped the link")
	}
	if strings.Contains(readExclude(t, root), "/harness\n") {
...
// TestModuleLinkRefusesTheCheckoutsOwnPath is a repository with its own harness
// directory where a module links: the link is hard, because permission rules and scripts
...
func TestModuleLinkRefusesTheCheckoutsOwnPath(t *testing.T) {
	...
	_, err = render.LinkModules(res, root, home, nil, true)
	...
	linked, err := render.LinkModules(res, root, home, nil, true)
	...
	if target, err := os.Readlink(filepath.Join(root, "harness")); err != nil || target != filepath.Join(".qory", "harness", "modules", "core") {
		t.Fatalf("harness links to %q (%v), want the module through the home", target, err)
	}
	if !strings.Contains(readExclude(t, root), "/harness\n") {
		t.Errorf("exclude file lacks /harness:\n%s", readExclude(t, root))
	}
...
// TestModuleLinkRefusesARuntimesPath is a stack that links a module where a runtime
// links, or at the qory directory: the compose refuses with a plain error, and a link
// the same stack asks for elsewhere is not written first.
func TestModuleLinkRefusesARuntimesPath(t *testing.T) {
	for _, c := range []struct{ link, want string }{
		...
		t.Run(c.link, func(t *testing.T) {
			res, root, home := composeFixture(t, "claude")
			linkModule(t, res, "core", "harness")
			linkModule(t, res, "nextjs", c.link)
			if err := render.Build(res, home, lookup(t, "claude")); err != nil {
				t.Fatal(err)
			}
			_, err := render.LinkModules(res, root, home, nil, false)
			...
			if errorsAs(err, &foreign) {
				t.Errorf("the refusal is a ForeignPathError, which the command module reads as a path in the way")
			}
...
// TestUnlinkModulesTakesOnlyQorysLinks is qory harness remove in a checkout with a module
// link, a link of the person's own, a directory and a name that holds nothing: only the
// module link goes, with its exclude line, and the removal names it.
func TestUnlinkModulesTakesOnlyQorysLinks(t *testing.T) {
	res, root, home := composeFixture(t, "claude")
	linkModule(t, res, "core", "harness")
	if err := render.Build(res, home, lookup(t, "claude")); err != nil {
		t.Fatal(err)
	}
	if _, err := render.LinkModules(res, root, home, nil, false); err != nil {
		t.Fatal(err)
	}
	write(t, filepath.Join(root, "docs", "README.md"), "ours\n")
	...
	removed, err := render.UnlinkModules(root, []string{"harness", "own", "docs", "missing"})
...
// TestUnlinkModuleLinksFindsTheLinksWithoutTheReport is a person who ran rm -rf .qory
// before qory harness remove: the report that named the module links is gone, and the
// remove still finds the link by its target, takes it with its exclude line, and leaves
// a symlink of the person's own alone.
func TestUnlinkModuleLinksFindsTheLinksWithoutTheReport(t *testing.T) {
	res, root, home := composeFixture(t, "claude")
	linkModule(t, res, "core", "harness")
	if err := render.Build(res, home, lookup(t, "claude")); err != nil {
		t.Fatal(err)
	...
	removed, err := render.UnlinkModuleLinks(root)
	if err != nil || len(removed) != 1 || removed[0] != "harness" {
		t.Fatalf("unlink: removed=%v err=%v", removed, err)
	}
	if _, err := os.Lstat(filepath.Join(root, "harness")); err == nil {
