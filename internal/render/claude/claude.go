...
//
// Claude Code also takes a harness from outside the checkout: the runtime's directory
// has the shape of its configuration directory, ~/.claude, and CLAUDE_CONFIG_DIR names
// it as such, so the skills, agents, commands, output styles and hooks under it register
// the way the user's own do, settings.json is read as the user's settings and CLAUDE.md
// as the user's memory. A plugin, --plugin-dir, registers the skills and commands of its
// layout but not its agents, so the harness is not delivered as one. The servers are the
// one thing the directory does not carry for Claude Code, and mcp.json goes to
// --mcp-config. [Template] holds those. Claude Code keeps its own state in the same
// directory, the login and the transcripts under projects, so a launcher sets those up
// once per home the way it would for a fresh ~/.claude.
//
...
func (claude) Render(res *compose.Result, dir, home string) error {
	...
	if res.Instructions != "" {
		return render.WriteFile(dir, "CLAUDE.md", []byte(res.Instructions))
	}
	return nil
}

// Template starts Claude Code with the harness in a home: the runtime's directory as
// CLAUDE_CONFIG_DIR, which registers the skills, agents, commands, output styles and
// hooks, and reads settings.json for the permissions, the environment and the model and
// CLAUDE.md for the instructions, as the user's; mcp.json through --mcp-config for the
// servers when the compose wrote one; and the setting sources cut to the user's, so no
// .claude of the checkout or a directory above it is read.
