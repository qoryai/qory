...
// outside it that the command module derives from the configuration. [Build] writes it in
// one step that either lands whole or leaves the previous home untouched: it stages the
// tree in a sibling directory, home with ".tmp" appended, puts the parts every runtime
// shares at its root, AGENTS.md, skills/, hooks/ and one modules/<name> link per module
// to the module's own directory, calls the runtime's Render for a subdirectory named
...
// the staging directory, and a settings file it writes contains the home, the path the
// runtime reads the tree from.
//
...
// # The links
//
// [LinkInto] creates in the checkout the paths the program looks at, relative symlinks so
// a checkout that moves keeps them valid. Every link and the .qory directory go into the
...
// directory containing one symlink per entry of the home's directory, so the checkout's
// own files in it stay, Claude Code's settings.local.json among them. A link is hard or
// soft:
//
...
// runtime passed to it also declares is left for that one. It refuses a path that is
// anything else, and for a soft link it passes over such a path in silence, which is how
// a checkout's own AGENTS.md survives a remove. Inside a linked directory only the
...
//
//   - [PlaceEntries] puts the atomic entries of the kinds it lists into the runtime's
...
//
// [Skipped] lists the composed entries of the kinds the runtime's Skips reports no place
...
// A runtime with no directory in the checkout, goose and any, lists the files kind in
// its Skips, and [Skipped] lists a files entry only for the runtime in its path.
//
...
// refuses a files entry at a reserved path, or one whose runtime segment is no
