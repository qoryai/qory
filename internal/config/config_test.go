...
func TestLoadRefusesAMistake(t *testing.T) {
	...
	for _, c := range []struct{ body, want string }{
		...
		{"worktree: {link: [{from: /etc/hosts, to: /etc/hosts}]}\n", `worktree.link sets to "/etc/hosts" for /etc/hosts, which is not a path inside the worktree`},
		{"worktree: {copy: [{from: /etc/hosts, to: ../hosts}]}\n", `worktree.copy sets to "../hosts" for /etc/hosts, which is not a path inside the worktree`},
		{"worktree: {copy: [{from: /etc/hosts, to: .}]}\n", `worktree.copy sets to "." for /etc/hosts, which is not a path inside the worktree`},
		...
		{"worktree: {link: [\"\"]}\n", `worktree.link lists an entry with no path`},
		{"worktree: {link: [{from: .env, to: ../.env}]}\n", `worktree.link sets to "../.env" for .env, which is not a path inside the worktree`},
