...
4. **A requirement that is not composed fails.** After the excludes and the collision
   ...
   deploy requires command ship, which module core leaves out` when an exclude dropped it,
   naming the module whose exclude did, and with `module core: skill deploy requires
