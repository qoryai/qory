...
harness:
  extends: {git: git@git.example.com:acme/harness, ref: main, path: nextjs-15}  # or left out for compose -f <stack>
...
`{path: <dir>}` or `{git: <url>, ref: <ref>, path: <dir>}`. `extends` may be left out
when `qory harness compose -f <stack>` names the base (§Discovery); a section with
neither `target` nor `extends`, composed with no base from `-f`, is refused with status
2, and the message says what the section holds instead. A section that extends a stack,
under `extends` or from `-f`, may name no modules and carry its extensions alone; a stack
names at least one module. `apiVersion` is optional in this file (§The configuration). A
`harness` section holding only the machine's keys, `runtime`, `model`, `force` and
`update`, is not a document. The schema is [config.schema.json](config.schema.json).
