...
`.gemini`. So every repository contains one copy per tool. When the shared part improves,
the copies drift, and nothing shows which version a checkout runs with.
...
and kept out of git. A report lists the module every entry came from.
...
entry, `qory` refuses to compose until the stack selects which one to keep. Nothing wins
by coming last.
...
1. Write `qory.yaml` in the repository, or let `qory setup repo` write it:
   ...
   Every module has a `qory-module.yaml` that defines its name. A repository can also
   take a stack someone else delivers, a `qory-stack.yaml`: it selects that stack under
   `extends`, defines what it composes it for under `target`, and adds its own modules.
   The delivered modules cannot be changed. A runner that has the stack tree selects the
   base with `qory harness compose -f <stack>` instead, and the repository's file then
   contains no version, ref or URL of it. The harness repository states the qory it needs
   once, in its own `qory.yaml`, `qory: ">=0.5.0"`, and that requirement applies to every
   stack it delivers.
   ...
   `stack` and `module` select what the harness repository publishes in the `exports`
...
Both modules ship a greet skill. The README that `qory setup example` writes lists the
...
image of yours that contains the agent; `qory` builds none. A minimal one for Claude Code:
...
and pass the run a policy that selects the credential, kept outside the checkout:
...
the repository's `qory.yaml` defines and composes the harness into it:
...
`--pr` sends no request to a hosting API: the pull request's head is one of the refs the
remote publishes, `refs/pull/<n>/head` on GitHub and Forgejo,
`refs/merge-requests/<n>/head` on GitLab, `refs/pull-requests/<n>/from` on Bitbucket
Server. `worktree.pr: refs/.../{n}/...` in the repository's `qory.yaml` defines the layout
of another host; a host that publishes none, Bitbucket Cloud, takes `--branch` with the
...
qory update              # install the newest release; --check only reports whether one exists
```
...
repository's file is committed and contains what the repository decides: its stack, and
what a worktree needs; it may be `harness.yaml` instead, a file name that does not mention
the tool that reads it. Your file, in `~/.config/qory`, defines how `qory` runs on this
machine for every repository: the runtime to compose for, where worktrees go, the git
timeout. The repository's file is read on top of yours. `qory config` shows every setting
and the file it came from. The reference, one page per command, is under
...
- Modules from a directory beside the repository, or from a git repository at a tag,
  ...
  them under `exports`, and a consumer selects them by name, never by directory.
- One instruction file, joined from the modules in order, under the name each tool wants.
- A report that lists the module of every entry. When two modules provide the same entry,
...
- Part of a module, when that is all you want. `exclude` leaves entries, the instruction
  section, settings fragments or variables out. `only` takes the listed things, plus what
...
The home, `.qory/harness`, contains one directory per runtime. Most of what is in it is a
...
per entry, so Claude Code's own `settings.local.json` stays beside them; `.mcp.json` and a
root `AGENTS.md` are symlinks. A tool that walks the tree has to follow them:

- `find` needs `-L`.
- `grep -R` with a symlink as its operand without a trailing slash reads nothing with
  BSD grep on macOS and follows the link with GNU grep on Linux, and neither reports an
  error. Pass it the directory with the slash, `grep -R pattern .claude/`, or use `-R -L`.
- `rg` needs `--follow`.
...
launcher, and the checkout then contains nothing of it: no link, no `.qory`, no exclude
line, so a tracked `harness/` directory is no collision and a pull is an ordinary pull.
`--home <dir>`, or `harness.home` in your own `qory.yaml`, defines the directory, and
...
the session's bytes. One optional file, `~/.config/qory/runner.yaml`, defines what the
...
`~/.config/qory/runtimes/<runtime>.yaml` describes another, or replaces the one shipped;
and a runtime nothing describes runs all the same, the run, its log and its egress
recorded and the session's own events not.
...
nothing. The policy alone defines what the runtime reaches.
...
answers: the events go where the configuration defines, and when it selects a run
configuration, that is the run's policy, fetched for the checkout's forge and repository
and reloaded when the server reports it changed. `--local` runs with the files alone and
...
it is; the `deny` lists of both apply either way. With a server configured the server's
...
settings against it. The credential role defines the credential whose name is the key,
...
by where it resolves. The rule applies to each run as it starts, so a program written into
a directory while that directory was mounted read-write is judged by where it is on every
run that follows; keep a program's directory out of the read-write mounts. The resolved
file, every directory above it up to `/`, and every directory above each link on the way,
the `PATH` directory among them, keep one rule: root or the user running `qory` owns each,
and each link on the way; other users may write none; and a group may write one when the
group is root's, gid 0, `wheel` or `admin`, or the owner's primary group when that group
has the owner's name. A directory root owns with the sticky bit set, `/tmp` or
`/nix/store`, keeps the rule. A default Homebrew install and a `~/go/bin` of a user's
private group keep it. `qory run` prints the program it found on a line of its own. The
...
leaves it undescribed. A run whose policy is on this machine describes the integrations
its `credentials` select; a run whose policy the server supplies describes every one. A
...
that contains the runtime; qory builds none. What to know:
...
- Inside the container the relay and the hook forwarder are qory's own Linux build. On
  ...
  release for your engine's architecture and set `wall.helper` to the binary.
- The container sees the checkout it was started in and no other directory, unless
  `--mount <path>[:ro]` or `wall.mounts` shows it one, at its own path, such as a
  sibling checkout the session reads. A socket is never mounted. In a git worktree the
...
- Behind a wall the proxy reaches your own machine only for a host `egress.allow`
  lists itself, in either mode, and never the cloud metadata address. For a local model
